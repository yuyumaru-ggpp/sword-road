...
try {
    ...
    foreach ($matchResults as $posName => $posData) {
        ...
        // ★★★ 選手IDがnullまたは空の場合はスキップ ★★★
        if (empty($redPlayerId) || empty($whitePlayerId)) {
            error_log("Warning: Player ID is empty for position {$posName}. Red: " . var_export($redPlayerId, true) . ", White: " . var_export($whitePlayerId, true));
            continue; // このポジションの挿入をスキップ
        }

        // さらに数値型に変換（安全のため）
        $redPlayerId = (int) $redPlayerId;
        $whitePlayerId = (int) $whitePlayerId;

        // ゼロチェック
        if ($redPlayerId <= 0 || $whitePlayerId <= 0) {
            error_log("Warning: Invalid player ID for position {$posName}. Red: {$redPlayerId}, White: {$whitePlayerId}");
            continue;
        }
        ...
        // 最終勝者
        $positionWinner = ($result['winner'] !== 'draw') ? $result['winner'] : null;
